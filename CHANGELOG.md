...
- Questions list filters: hidden-by-default filter panel with tag datalist suggestions, tag query support, and pagination-safe tag/search params.
...
- Admin-only weekly MVP metrics report endpoint: `GET /api/admin/metrics/weekly` (TTFV, participation, resolution rate, loop-velocity proxies).
